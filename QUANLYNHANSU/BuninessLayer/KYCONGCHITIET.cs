...
namespace BuninessLayer
{
    public class KYCONGCHITIET
    {
        ...
        // dòng 14-17 lấy da đc danh sách ngày công của 1 nv
        ...
        public List<tb_KYCONGCHITIET> getList(int makycong)// lấy danh sách
        {
            return db.tb_KYCONGCHITIET.Where(x => x.MAKYCONG == makycong).ToList();
            // lấy dữ liệu data nvà trả về "MAKYCONG" bằng với giá trị của biến "makycong".
        }

        // hàm phát sinh kỳ công ( bảng công)
        public void phatSinhKyCongChiTiet(int macty, int thang, int nam)
        {
            ...
            foreach (var item in lstNV)
            {
                ...
                for (int j = 1; j <= GetDayNumber(thang, nam); j++) // dùng để lấy ngày trong tháng
                {
                    DateTime newDate = new DateTime(nam, thang, j); // trả về ngày trong tuần

                    switch (newDate.DayOfWeek.ToString())
                    {
                        case "Sunday":
                            listDay.Add("CN");// nếu là chủ nhật thì để cn
                            break;
                ...
                switch (listDay.Count)// thêm cột cho đủ 31 ngày công
                {
                    case 28:
                        listDay.Add("");
                        listDay.Add("");
                ...
                kycongchitiet.NGAYCONG = MyFuntion.demSoNgayLamViecTrongThang(thang, nam);// đếm xem có bn lm việc trong tháng
                kycongchitiet.TONGNGAYCONG = MyFuntion.demSoNgayLamViecTrongThang(thang, nam);
